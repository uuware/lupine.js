/**
 * Converted from PHP classes/UpdateEntityHits.php
 */
...
export class UpdateEntityHits extends ProcessBase {
  ...
  override async execute(): Promise<boolean | void> {
    ...
    try {
      await db.execute('UPDATE ' + realTblid + ' SET hits = hits + 1' + filter);
    } catch(e: any) {
