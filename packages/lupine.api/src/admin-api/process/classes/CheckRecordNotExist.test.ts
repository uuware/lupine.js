...
describe('CheckRecordNotExist Process', () => {
  it('CheckRecordNotExist accepts when no matching row exists', async () => {
    ...
    const db = makeMockDb([[]]);
    const proc = new CheckRecordNotExist();
  ...
  it('CheckRecordNotExist rejects when a matching row exists', async () => {
    const keyentity = makeEntityObject('$__users', [makePhysicalFieldObject('email', 'email', 'alice@example.com')]);
    const entity = makeEntityObject('$__users', [makePhysicalFieldObject('email', 'email')]);
    ...
    assert.match(entity.getErrorMsg(), /SE_EXISTREC/);
    assert.deepEqual(db.selects[0].params, ['alice@example.com']);
  });

  it('CheckRecordNotExist supports LIKE key filters with escaped parameters', async () => {
  ...
  it('CheckRecordNotExist rejects missing table id from entity', async () => {
    const keyentity = makeEntityObject('$__users', [makePhysicalFieldObject('email', 'email', 'alice@example.com')]);
  ...
  it('CheckRecordNotExist returns false and reports query errors', async () => {
    const keyentity = makeEntityObject('$__users', [makePhysicalFieldObject('email', 'email', 'alice@example.com')]);
    const entity = makeEntityObject('$__users', [makePhysicalFieldObject('email', 'email')]);
    const db = makeMockDb();
  ...
  it('runProcess — accepts when no matching row exists', async () => {
    const items: ItemDef[] = [
