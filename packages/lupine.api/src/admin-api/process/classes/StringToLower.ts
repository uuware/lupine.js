/**
 * Converted from PHP classes/StringToLower.php
 */

import { ProcessBase } from '../process-base';
import { FieldObject, VectorObject, FieldType } from '../field-objects';

export class StringToLower extends ProcessBase {
  ...
  override execute(): boolean | void {
    ...
    if(this.fields instanceof VectorObject) {
      ...
      for (let i = 0; i < cnt; i++) {
        ...
        item?.setValue(String(item.getValue() || '').toLowerCase());
      }
    }
    else {
      const field = this.fields as FieldObject;
      field?.setValue(String(field.getValue() || '').toLowerCase());
    }
  }
}
