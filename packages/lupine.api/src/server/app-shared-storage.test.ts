...
test('AppSharedStorage - IPC Concurrency & Async Callbacks', async (t) => {
  ...
  await t.test('Primary file load/save with Mutex concurrency', async () => {
    ...
    try {
      // 3. Test Load
      // Pre-create the resources/config_default.json to prevent the logger from screaming ERROR for ENOENT during the test
      const resDir = path.join(tempDir, 'resources');
      await fs.mkdir(resDir, { recursive: true });
      await fs.writeFile(path.join(resDir, 'config_default.json'), '{}', 'utf8');
