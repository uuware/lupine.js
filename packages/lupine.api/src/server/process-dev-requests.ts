...
export const processDebugMessage = async (msgObject: any) => {
  ...
  if (msgObject.id === 'debug' && msgObject.message === 'refresh') {
    ...
    DevServiceProxyServer.broadcastRefresh && DevServiceProxyServer.broadcastRefresh();
  }
...
export async function processShell(req: ServerRequest) {
  ...
  const shell = DevServiceProxyServer.shellDirectCmd ? await DevServiceProxyServer.shellDirectCmd(cmd) : '';
  return shell;
}
...
ServerControlProxyServer.processRefreshCache = processRefreshCache;
ServerControlProxyServer.processRestartApp = processRestartApp;
ServerControlProxyServer.processShell = processShell;
