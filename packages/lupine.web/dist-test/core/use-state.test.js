...
var getCurrentStore = () => _currentStore;
...
function useState(initial) {
  ...
  const setState = (newVal) => {
    ...
    const resolved = typeof newVal === "function" ? newVal(store.states[idx]) : newVal;
...
// src/core/use-state.test.ts
var import_node_test = require("node:test");
var import_strict = __toESM(require("node:assert/strict"));
(0, import_node_test.describe)("use-state Core Engine", () => {
  ...
  (0, import_node_test.describe)("evaluateComponentWithStore", () => {
    (0, import_node_test.it)("should successfully evaluate a synchronous component and return VNode", async () => {
      const SyncComp = (props) => {
      ...
      import_strict.default.deepEqual(result.props.children, ["Sync World"]);
    });
    (0, import_node_test.it)("should successfully evaluate an asynchronous component and return VNode", async () => {
      const AsyncComp = async (props) => {
        await new Promise((r) => setTimeout(r, 1));
        return { type: "span", props: { children: [props.text] } };
      };
      const store = new ComponentStateStore(AsyncComp, { text: "Async World" });
      const result = await evaluateComponentWithStore(store);
      import_strict.default.equal(result.type, "span");
      import_strict.default.deepEqual(result.props.children, ["Async World"]);
    });
    (0, import_node_test.it)("should set and clear the global currentStore during execution", async () => {
      let storeInsideComp = void 0;
      const TestComp = () => {
        storeInsideComp = getCurrentStore();
        return { type: "div", props: {} };
      };
      import_strict.default.equal(getCurrentStore(), null);
      const store = new ComponentStateStore(TestComp, {});
      await evaluateComponentWithStore(store);
      import_strict.default.equal(storeInsideComp, store);
      import_strict.default.equal(getCurrentStore(), null);
    });
    (0, import_node_test.it)("should inject ref wrapper onto VNode if a hook is used", async () => {
      const HookComp = () => {
        useState(0);
      ...
      const result = await evaluateComponentWithStore(store);
      import_strict.default.ok(result.props.ref, "Ref should be injected because a hook was used");
      import_strict.default.equal(typeof result.props.ref.onLoad, "function");
      import_strict.default.equal(typeof result.props.ref.refresh, "function");
    });
    (0, import_node_test.it)("should inject ref wrapper onto VNode if user already passed a ref", async () => {
      ...
      const result = await evaluateComponentWithStore(store);
      import_strict.default.ok(result.props.ref);
      import_strict.default.equal(result.props.ref, userRef, "It should merge into the original user ref object");
      import_strict.default.equal(result.props.ref.globalCssId, "test-global-css");
      import_strict.default.equal(typeof result.props.ref.onLoad, "function", "Wrapper added successfully");
    });
  });
});
