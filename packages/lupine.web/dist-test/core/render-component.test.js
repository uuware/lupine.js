...
// src/core/render-component.test.ts
...
// src/core/render-component.test.ts
(0, import_node_test.describe)("renderComponentAsync", () => {
  (0, import_node_test.beforeEach)(() => {
    import_node_test.mock.method(Date, "now", () => 1775190809080);
    domUniqueId(true);
  });
  (0, import_node_test.afterEach)(() => {
    import_node_test.mock.restoreAll();
  });
  const getHtml = (props) => props._html.join("");
  (0, import_node_test.describe)("Basic HTML Elements Rendering", () => {
    (0, import_node_test.it)("should render standard HTML tags without attributes", async () => {
      const props = { _html: [] };
      await renderComponentAsync("div", props);
      import_strict.default.equal(getHtml(props), "<div></div>");
    });
    (0, import_node_test.it)("should render self-closing tags correctly", async () => {
      const props = { _html: [], src: "image.png", alt: "logo" };
      await renderComponentAsync("img", props);
      import_strict.default.equal(getHtml(props), '<img src="image.png" alt="logo" />');
    });
  });
  (0, import_node_test.describe)("Attributes Processing", () => {
    (0, import_node_test.it)("should correctly render basic attributes", async () => {
      const props = { _html: [], id: "main", "data-test": "value", role: "button" };
      await renderComponentAsync("section", props);
      import_strict.default.equal(getHtml(props), '<section id="main" data-test="value" role="button"></section>');
    });
    (0, import_node_test.it)("should convert object style property to inline CSS string", async () => {
      const props = { _html: [], style: { backgroundColor: "red", fontSize: "12px" } };
      await renderComponentAsync("div", props);
      import_strict.default.equal(getHtml(props), '<div style="background-color:red;font-size:12px;"></div>');
    });
    (0, import_node_test.it)("should process default boolean attributes (disabled, readonly, selected, checked)", async () => {
      const props = { _html: [], disabled: true, readonly: "true", checked: false, selected: "selected" };
      await renderComponentAsync("input", props);
      import_strict.default.equal(getHtml(props), '<input disabled="true" readonly="true" selected="selected" />');
    });
    (0, import_node_test.it)('should handle defaultChecked converting it to checked="true"', async () => {
      const props = { _html: [], defaultChecked: true };
      await renderComponentAsync("input", props);
      import_strict.default.equal(getHtml(props), '<input checked="true" />');
    });
  });
  (0, import_node_test.describe)("Children Content Rendering", () => {
    (0, import_node_test.it)("should successfully render string and number children", async () => {
      const props = { _html: [], children: ["Balance: ", 100] };
      await renderComponentAsync("div", props);
      import_strict.default.equal(getHtml(props), "<div>Balance: 100</div>");
    });
    (0, import_node_test.it)("should gracefully ignore false, null, and undefined in children array", async () => {
      const props = { _html: [], children: [false, null, void 0, "visible text"] };
      await renderComponentAsync("span", props);
      import_strict.default.equal(getHtml(props), "<span>visible text</span>");
    });
    (0, import_node_test.it)("should recursively render nested component children tree", async () => {
      ...
      import_strict.default.equal(getHtml(props), "<p>A <strong>bold content</strong> end.</p>");
    });
  });
  (0, import_node_test.describe)("DangerouslySetInnerHTML Logic", () => {
    (0, import_node_test.it)("should render raw HTML correctly", async () => {
      const props = { _html: [], dangerouslySetInnerHTML: "<span>raw injected string</span>" };
      await renderComponentAsync("div", props);
      import_strict.default.equal(getHtml(props), "<div><span>raw injected string</span></div>");
    });
  });
  (0, import_node_test.describe)("Functional Components Resolution", () => {
    (0, import_node_test.it)("should execute synchronous functional components", async () => {
      ...
      await renderComponentAsync(MockComponent, props);
      import_strict.default.equal(getHtml(props), "<h1>Lupine Test</h1>");
    });
    (0, import_node_test.it)("should await and execute asynchronous functional components", async () => {
      const AsyncMockComponent = async (p) => {
      ...
      const props = { _html: [] };
      await renderComponentAsync(AsyncMockComponent, props);
      import_strict.default.equal(getHtml(props), "<span>Data Loaded</span>");
    });
  });
  (0, import_node_test.describe)("Fragment and Virtual Wrapper Handling", () => {
    (0, import_node_test.it)("should omit wrappers when using string type Fragment", async () => {
      const props = { _html: [], children: ["fragment inner text"] };
      await renderComponentAsync("Fragment", props);
      import_strict.default.equal(getHtml(props), "fragment inner text");
    });
    (0, import_node_test.it)("should omit wrapper when using named Fragment function", async () => {
      const FragmentFunc = () => {
      };
      Object.defineProperty(FragmentFunc, "name", { value: "Fragment" });
      const props = { _html: [], children: ["named function wrapper"] };
      await renderComponentAsync(FragmentFunc, props);
      import_strict.default.equal(getHtml(props), "named function wrapper");
    });
  });
  (0, import_node_test.describe)("CSS Mapping and Unique ID Implementations", () => {
    (0, import_node_test.it)("should inject global and unique IDs into the class attribute matching ampersand symbol", async () => {
      const props = { _html: [], class: "base-cls &-item" };
      await renderComponentAsync("div", props, "u-id", "g-id");
      import_strict.default.equal(getHtml(props), '<div class="base-cls g-id-item u-id-item"></div>');
    });
    (0, import_node_test.it)("should automatically prepend unique generated ID to class list when local CSS is present", async () => {
      const props = { _html: [], css: { ".&-item": { color: "block" } } };
      await renderComponentAsync("div", props);
      const output = getHtml(props);
      import_strict.default.equal(output, '<div class="l00" l00><style id="sty-l00">.l00 .l00-item{color:block;}</style></div>');
    });
  });
});
