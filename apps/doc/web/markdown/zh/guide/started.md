...
我们在项目根目录提供了一个专属的 `AI_CONTEXT.md` 文件。当请求 AI 为本项目编写代码时，请先将此文件提供给 AI。其中包含了 Lupine.js 独有的关键规则（例如“禁用 React Hooks”）和设计模式。

#### 提示词示例 (Example Prompt)

> "请先阅读 `AI_CONTEXT.md`。创建一个‘用户资料’组件，包含标准的设置列表（头像、姓名、邮箱）和一个保存按钮。"
